...
static LOCALETAB __rg_language[] =
{
    {"american",                    "ENU"},
...
static LANGID rglangidNotDefault[] =
{
...
static BOOL TestDefaultCountry (LCID lcid)
{
    ...
    for (i = 0; i < sizeof(rglangidNotDefault) / sizeof(LANGID); i++)
    {
        if (langid == rglangidNotDefault[i])
            return FALSE;
    }
    return TRUE;
}
                                                                  ...
                                                                  int cchdata)
{
    int     i;
    int     low = 0;
    int         high = sizeof(rgLocInfo) / sizeof(RGLOCINFO) - 1;
    char *      pchResult = NULL;
    ...
    {
        i = (low + high) / 2;
        if (lcid == rgLocInfo[i].lcid)
        {
            //  LCID matched - test for valid LCTYPE to simulate call
            switch (lctype)
            {
                case LOCALE_ILANGUAGE:
                    pchResult = rgLocInfo[i].chILanguage;
                    break;
                case LOCALE_SENGLANGUAGE:
                    pchResult = rgLocInfo[i].pchSEngLanguage;
                    break;
                case LOCALE_SABBREVLANGNAME:
                    pchResult = rgLocInfo[i].chSAbbrevLangName;
                    break;
                case LOCALE_SENGCOUNTRY:
                    pchResult = rgLocInfo[i].pchSEngCountry;
                    break;
                case LOCALE_SABBREVCTRYNAME:
                    pchResult = rgLocInfo[i].chSAbbrevCtryName;
                    break;
                case LOCALE_IDEFAULTCODEPAGE:
                    pchResult = rgLocInfo[i].chIDefaultCodepage;
                    break;
                case LOCALE_IDEFAULTANSICODEPAGE:
                    pchResult = rgLocInfo[i].chIDefaultAnsiCodepage;
                default:
                    break;
            }
