...
_AfxCbtFilterHook(int code, WPARAM wParam, LPARAM lParam)
{
   ...
   TCHAR szClassName[5];  // Just enough to hold "IME"+one extra char+NUL.
                          // This will be enough to differentiate between "IME"
                          // and names that begin with IME.

   if (HIWORD((DWORD)lpcs->lpszClass))
   {
   ...
   {
      szClassName[0] = _T('\0');
      GlobalGetAtomName( (ATOM)(DWORD)(lpcs->lpszClass), szClassName, 
         sizeof(szClassName)/sizeof(szClassName[0]));
      pszClassName = szClassName;
   }

	if ((!(GetClassLong((HWND)wParam, GCL_STYLE)&CS_IME) && _tcscmp(
      pszClassName, _T("IME")) != 0) && (pWndInit != NULL || (!(lpcs->style & 
      WS_CHILD) && !afxContextIsDLL)))
	{
...
void CWnd::OnNcDestroy()
{
	...
	if (m_nFlags & WF_TOOLTIPS)
	{
		...
		if (pToolTip->GetSafeHwnd() != NULL)
		{
			...
			ti.cbSize = sizeof(AfxOldTOOLINFO);
